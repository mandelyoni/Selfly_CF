...
void positionControllerInit()
{
	...
	this.pidZ.init.kp = 2.0;
	this.pidZ.init.ki = 0.5;
	this.pidZ.init.kd = 0;
	this.pidZ.setpoint = 0;
	
	this.pidVZ.init.kp = 25.0;
	this.pidVZ.init.ki = 15.0;
	this.pidVZ.init.kd = 0;
	this.pidVZ.setpoint = 0;
	
	this.thrustBase = 36000;
	this.thrustMin = 20000;
...
void velocityController(float* thrust, attitude_t *attitude, setpoint_t *setpoint,
                                                             const state_t *state)
{
	float rollRaw, pitchRaw, thrustRaw;
	float yawRad;
...
*/
  ...
  yawRad = state->attitude.yaw * (float)M_PI / 180;
  ...
  *thrust = thrustRaw*thrustScale + this.thrustBase; // thrustScale = 1000;  thrustBase = 36000;
  // Check for minimum thrust
  if (*thrust < this.thrustMin)  // thrustMin = 20000
	{
