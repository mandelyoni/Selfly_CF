...
{ // called from estimatorComplementary
...
// 02/10/2011 SOH Madgwick  Optimised for reduced CPU load
static void sensfusion6UpdateQImpl(float gx, float gy, float gz, float ax, float ay, float az, float dt)
{ // called from sensfusion6UpdateQ (this file)
  float recipNorm;
...
static void estimatedGravityDirection(float* gx, float* gy, float* gz)
{ // second function called from sensfusion6UpdateQ  :     estimatedGravityDirection(&gravX, &gravY, &gravZ); 
  *gx = 2 * (q1 * q3 - q0 * q2);
  *gy = 2 * (q0 * q1 + q2 * q3);
