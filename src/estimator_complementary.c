...
void estimatorComplementary(state_t *state, sensorData_t *sensorData, control_t *control, const uint32_t tick)
{
  ...
  /*
	if (RATE_DO_EXECUTE(ATTITUDE_UPDATE_RATE, tick))  // ATTITUDE_UPDATE_RATE RATE_250_HZ
  {
  ...
  //*qw = q0;
    ...
    positionUpdateVelocity(state->acc.z, ATTITUDE_UPDATE_DT); //ATTITUDE_UPDATE_RATE = RATE_250_HZ
  }
