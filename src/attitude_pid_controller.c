       ...
       float rollRateDesired, float pitchRateDesired, float yawRateDesired)
{
	...
	float rollOutput_temp, pitchOutput_temp, yawOutput_temp;
	
  pidSetDesired(&pidRollRate, rollRateDesired);
  rollOutput_temp = (pidUpdate(&pidRollRate, rollRateActual, true)); //saturateSignedInt16
	if (rollOutput_temp > INT16_MAX)
    rollOutput =  INT16_MAX;
  else if (rollOutput_temp < -INT16_MAX)
    rollOutput =  -INT16_MAX;
  else
    rollOutput =  (int16_t)rollOutput_temp;

  pidSetDesired(&pidPitchRate, pitchRateDesired);
  pitchOutput_temp = (pidUpdate(&pidPitchRate, pitchRateActual, true)); // saturateSignedInt16
	if (pitchOutput_temp > INT16_MAX)
    pitchOutput =  INT16_MAX;
  else if (pitchOutput_temp < -INT16_MAX)
    pitchOutput =  -INT16_MAX;
  else
    pitchOutput =  (int16_t)pitchOutput_temp;

  pidSetDesired(&pidYawRate, yawRateDesired);
  yawOutput_temp = (pidUpdate(&pidYawRate, yawRateActual, true)); // saturateSignedInt16
	if (yawOutput_temp > INT16_MAX)
    yawOutput =  INT16_MAX;
  else if (yawOutput_temp < -INT16_MAX)
    yawOutput =  -INT16_MAX;
  else
    yawOutput =  (int16_t)yawOutput_temp;
}
